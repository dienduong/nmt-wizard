...
@app.route("/v2/evaluation", methods=["POST"])
# @filter_request("POST/v2/task/launch", "train")
def evaluation():
    service = GLOBAL_POOL_NAME
    entity_code = g.user.entity.entity_code
    ...
    # models = ["SAADM_enfr_model.1_01_ea10295beb4649a7837-33c6d", "SAADM_enfr_model.1_01_ed47b2873f9c4d4dbb1-6350a"]
    # corpus = ["test_1.tmx", "test_2.tmx"]
    ...
    to_translate_corpus = get_to_score_corpus(corpus, upload_path, source_language, target_language, storage_id)
    to_score_corpus = get_to_score_corpus(corpus, upload_path, source_language, target_language, storage_id)

    model_task_map = {}

    for model in models:
        # translation_docker_image = get_docker_image_info(service_module, entity_owner, "systran/pn9_tf")
        other_task_infos = {
        ...
        # scoring_docker_image = get_docker_image_info(service_module, entity_owner, "nmtwizard/score")
...
def parse_request_data_of_evaluation(request):
    ...
    return {**request_data, **{"models": models}, **{
...
def validate_request_data_of_evaluation(request):
    ...
    # models: training completed? same lp? num of models?
